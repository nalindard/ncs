...
<template>
    <span class="w-5/12 sm:w-4/12 md:w-3/12 lg:w-2/12 m-auto text-center flex justify-evenly relative border-t border-l border-blue-200 py-1 sm:py-2 md:py-4 rounded-full">
        <!-- {{ getSong.id }} -->
        <!-- Like -->
        <button @click="" class="scale-[60%] md:scale-105 lg:scale-125">
            <i class="fa-regular fa-heart fa-lg"></i>
        </button>
        <!-- Play / Pause now -->
        <button @click="updateCurrentPlayList(song)" class="scale-[60%] md:scale-105 lg:scale-125">
            <i class="fa-solid fa-lg" :class="isThePlayingSong ? 'fa-pause' : 'fa-play'"></i>
        </button>
        <!-- Append -->
        <button @click="addToCurrentPlayList(song)" class="scale-[60%] md:scale-105 lg:scale-125">
            <i class="fa-solid fa-plus fa-lg"></i>
        </button>
        <!-- Download -->
        <button  class="scale-[60%] md:scale-105 lg:scale-125">
